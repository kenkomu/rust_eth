...
async fn main() -> Result<()> {
    ...
    let mnemonic = "gas monster ski craft below illegal discover limit dog bundle bus artefact";//create a mmemonic
    let ganache = Ganache::new().mnemonic(mnemonic).spawn();//create a ganache instance
    println!("HTTP Endpoint: {}", ganache.endpoint());//print out the ganache endpoint
    ...
    let provider = Provider::try_from(ganache.endpoint())?.interval(Duration::from_millis(10));//create a provider from ganache endpoint
    let chain_id = provider.get_chainid().await?.as_u64();//chain_id on which we will have to deploy
    ...
    let block = provider
        .clone()
        .get_block(BlockNumber::Latest)//get the latest block
        .await?
    ...
    let contract = deployer.clone().legacy().send().await?;//give you the contract object
